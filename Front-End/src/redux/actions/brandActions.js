...
export const loadBrands = (term = '') => {
    return (dispatch) => {
        //ก่อนดึงข้อมูลสั่ง dispatch ให้ reducer รู้ว่าก่อนเพื่อจะแสดง loading
        dispatch({ type: 'LOAD_BRANDS_PENDING' })
        return axios.get(`${BASE_URL}/brands?term=${term}`, {
            ...
            dispatch({ type: 'LOAD_BRANDS_SUCCESS', payload: results.data })
        }).catch(err => {
            //กรณี error
            dispatch({ type: 'LOAD_BRANDS_REJECTED', payload: err.message })
        })
    }
}
...
export const getBrand = (id) => {
    return (dispatch) => {
        dispatch({ type: 'LOAD_BRAND_PENDING' })
        return axios.get(`${BASE_URL}/brands/${id}`, {
            //ต้องส่ง heder ชื่อ authorization โดยส่ง token เขาไป
            //เพื่อบอกให้ server รู้ว่าเราได้ signin ถูกต้องแล้ว
            headers: { authorization: localStorage.getItem('token') }
        }).then(results => {
            ...
            dispatch({ type: 'LOAD_BRAND_SUCCESS', payload: results.data })
        }).catch(err => {
            //กรณี error
            dispatch({ type: 'LOAD_BRAND_REJECTED', payload: err.message })
        })
    }
}
    ...
    return (dispatch) => {
        ...
        return axios({
            ...
            if (results.data.status) {
                dispatch({ type: 'SAVE_BRAND_REJECTED', payload: results.data.message })
            } else {
                dispatch({ type: 'SAVE_BRAND_SUCCESS' })
            }
        }).catch(err => {
            //กรณี error
            dispatch({ type: 'SAVE_BRAND_REJECTED', payload: err.message })
        })
    }
}
...
export const deleteBrand = (id) => {
    return (dispatch) => {
        return axios.delete(`${BASE_URL}/beands/${id}`, {
            //ต้องส่ง heder ชื่อ authorization โดยส่ง token เขาไปด้วยครับ
            headers: { authorization: localStorage.getItem('token') }
        }).then(results => {
            //ลบข้อมูลสำเร็จ
            ...
            //กรณี error
            dispatch({ type: 'DELETE_BRAND_REJECTED', payload: err.message })
        })
    }
}
...
export const resetStatus = () => {
    return (dispatch) => {
        dispatch({ type: 'SAVE_BRABD_SUCCESS' })
    }
}